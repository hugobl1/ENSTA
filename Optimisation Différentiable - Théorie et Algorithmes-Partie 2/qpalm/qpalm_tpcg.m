function [prec] = qpalm_tpcg (prec)

%%
% [prec] = qpalm_tpcg (prec)
%
% Qpalm_tpcg is a truncated preconditioned conjugate gradient solver for
% the convex piecewise quadratic optimization problem
%
%   minimize (in x)  phi(x)
...
% qpalm_phi.
%
% Qpalm_tpcg uses preconditioned conjugate gradient iterations to
...
%if norm(M-MF)>1.e-10; keyboard; end
    if prec.rank < 0
      ...
      if verb > 0, fprintf(fout,'\n\n### qpalm_tpcg: the QP is nonconvex\n'); end
...
%Hessien=QPALM_H(VB,VB)
%Precon=prec.L*prec.L'
  while 1

%fprintf(fout,'\n\n i      l(i)          x(i)          u(i)');
%for i=1:QPALM_n
%  fprintf(fout,'\n%2i  %12.5e  %12.5e  %12.5e',i,QPALM_lbx(i),QPALM_x(i),QPALM_ubx(i));
%  if QPALM_x(i) <= QPALM_lbx(i)+QPALM_options.dxmin; fprintf(fout,'  bound-'); end
%  if QPALM_x(i) >= QPALM_ubx(i)-QPALM_options.dxmin; fprintf(fout,'  bound+'); end
%end
%fprintf(fout,'\n\n');
...
%err=Precon*pg-gVB
    ...
    phi = phi + alpha*(gphi'*d) + (alpha^2/2)*pd;
...
%fprintf('\n     lbi            y            ubi\n')
%for i = 1:QPALM_mi
%  fprintf('%12.5e  %12.5e  %12.5e\n',QPALM_lbi(i),QPALM_y(i)+QPALM_lmi(i)/r,QPALM_ubi(i))
%end
%PHI = qpalm_al ()
